...
# endif
#elif defined(BUMPALLOC)
   res = (BitVector) bump_alloc(msize);
#else		 
...
# endif		 
#elif defined(BUMPALLOC)
   SolnPtr bp = (SolnPtr) bump_alloc(sizeof (Soln));
#else		 
...
# endif		 
#elif defined(BUMPALLOC)
   res = (uvec) bump_alloc(count * sizeof (unsigned));
#else		 
...
# endif		 
#elif defined(BUMPALLOC)
   eas = (EasPtr) bump_alloc((aborts+1) * sizeof (EasEntry));
#else		 
...
# endif		 
#elif defined(BUMPALLOC)
   e = (solnvec) bump_alloc((m+1) * sizeof e[0]); 
#else		 
